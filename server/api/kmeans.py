...
class KMeansWrapper(object):
    ...
    def cluster(self, num_clusters: int) -> None:
        """Calculate num_clusters many cluster centers using KMeans. Access labels via api.kmeans.get_instance().labels,
        cluster centers via api.kmeans.get_instance().cluster_centers
